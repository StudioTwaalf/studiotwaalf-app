...
interface Props { params: { id: string; variantId: string } }

export default async function EditVariantPage({ params }: Props) {
  const [product, variant] = await Promise.all([
    prisma.product.findUnique({ where: { id: params.id } }),
    prisma.gadgetVariant.findUnique({ where: { id: params.variantId } }),
  ])
  ...
  return (
    <div className="max-w-2xl">
      <nav className="flex items-center gap-2 text-sm text-gray-400 mb-6">
        ...
        <span>/</span>
        <span className="text-gray-700 font-medium">{variant.name ?? 'Variant bewerken'}</span>
      </nav>

      <div className="bg-white rounded-2xl border border-gray-200 p-6">
        <h1 className="text-lg font-semibold text-gray-900 mb-6">Variant bewerken</h1>
        ...
        <form action={boundUpdate} className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  step="0.01" min="0"
                  defaultValue={variant.priceCents != null ? (variant.priceCents / 100).toFixed(2) : ''}
          ...
          <section>
            ...
            <div className="grid grid-cols-3 gap-4">
              <div>
                ...
                <input id="widthMm" name="widthMm" type="number" step="0.1" min="0.1"
              ...
              <div>
                ...
                <input id="heightMm" name="heightMm" type="number" step="0.1" min="0.1"
              ...
              <div>
                ...
                <input id="depthMm" name="depthMm" type="number" step="0.1" min="0"
                  defaultValue={variant.depthMm ?? ''}
                  placeholder="bijv. 40"
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
              </div>
              <div>
                ...
                <input id="mockupScale" name="mockupScale" type="number" step="0.05" min="0.3" max="3.0"
                  defaultValue={variant.mockupScale ?? ''}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
              </div>
              <div>
                ...
                <input id="visualPadding" name="visualPadding" type="number" step="0.01" min="0" max="0.45"
                  defaultValue={variant.visualPadding ?? ''}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
              </div>
              <div>
          ...
          <section>
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                { name: 'isActive',  label: 'Actief',            checked: variant.isActive,  hint: 'Toon deze variant in de gadgetpicker' },
                { name: 'isDefault', label: 'Standaard variant', checked: variant.isDefault, hint: 'Wordt automatisch geselecteerd bij toevoegen' },
              ].map((item) => (
                <label key={item.name}
        ...
        <div className="mt-8 pt-6 border-t border-gray-100">
          <h2 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-3">Gevaar</h2>
